...
public class GeoObjectAdapter extends RecyclerView.Adapter<GeoObjectViewHolder> {
    ...
    public void removeItem(int postition) {
        listGeoObject.remove(postition);
        notifyItemRemoved(postition);
        notifyItemRangeChanged(postition,listGeoObject.size());
    }

    public String checkInEurope(int position) {
        return listGeoObject.get(position).getmInEurope();
    }
